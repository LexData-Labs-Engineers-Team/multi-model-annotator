...
EDGE_THRESH         = 0.5       # edge connectivity confidence
TAG_THRESH          = cfg.TAG_SCORE_THRESH

# ---- Fusion MLP (Option 3) ----
# Set USE_FUSION_MLP = False to disable without removing the path.
# Train fusion_mlp/train_fusion_mlp.py before enabling.
FUSION_MLP_PATH     = os.path.join(cfg.SAVE_DIR,
                                   "fusion_mlp", "fusion_mlp_best.pt")
FUSION_MLP_THRESH   = 0.3    # drop predictions below this after re-scoring
USE_FUSION_MLP      = True   # set False to skip MLP fusion
...
def run_keypoints(model, img_path, orig_h, orig_w, device):
    ...
    local_max = (heatmap == maximum_filter(heatmap, size=15))
...
def main():
    ...
    # --- Fusion MLP ---
    fusion_mlp_model  = None
    fusion_mlp_mode   = None
    fusion_mlp_idim   = None
    if USE_FUSION_MLP and model_exists(FUSION_MLP_PATH):
        print(f"  ✓ Fusion MLP      : {FUSION_MLP_PATH}")
        sys.path.insert(0, os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "fusion_mlp"
        ))
        from train_fusion_mlp import load_fusion_mlp, apply_fusion_mlp
        fusion_mlp_model, fusion_mlp_mode, fusion_mlp_idim = \
            load_fusion_mlp(FUSION_MLP_PATH)
        fusion_mlp_model = fusion_mlp_model.to(device)
        print(f"    Mode: {fusion_mlp_mode}")
    elif USE_FUSION_MLP:
        print(f"  ✗ Fusion MLP      : not found — skipped")
        print(f"    Train fusion_mlp/train_fusion_mlp.py first.")
    else:
        print(f"  - Fusion MLP      : disabled (USE_FUSION_MLP=False)")
    ...
    for img_path in image_paths:
        ...
        # --- Apply Fusion MLP inline ---
        if fusion_mlp_model is not None and all_preds:
            coco_style = []
            for i, p in enumerate(all_preds):
                ...
                if p["type"] == "bbox":
                    x1, y1, x2, y2 = p["box"]
                    ann["bbox"] = [x1, y1, x2 - x1, y2 - y1]
                elif p["type"] in ("polygon", "polyline"):
                    flat = [v for pt in p["points"] for v in pt]
                    ann["segmentation"] = [flat]
                elif p["type"] == "keypoint":
                    ann["keypoints"] = [p["x"], p["y"], 2]
                coco_style.append(ann)

            refined = apply_fusion_mlp(
                fusion_mlp_model, coco_style,
                orig_w, orig_h,
                fusion_mlp_mode, device,
                score_thresh=FUSION_MLP_THRESH
            )
            kept_ids  = {r["id"] for r in refined}
            score_map = {r["id"]: r["score"] for r in refined}
            fused_preds = []
            for i, p in enumerate(all_preds):
                if i in kept_ids:
                    p = dict(p)
                    p["score_original"] = p.get("score", 1.0)
                    p["score"]          = score_map[i]
                    fused_preds.append(p)
            all_preds = fused_preds
